...
d3.json("patient_data.json").then(data => {
    // Compute statistics
    const totalPatients = data.length;
    const clusterCounts = d3.rollup(data, v => v.length, d => d.Cluster);
    ...
    function highlightNeighbors(selectedPatient) {
        const neighborCount = 5;  // Default neighbor count
        ...
        function calculateDistance(a, b) {
            if (distanceMetric === "euclidean") {
                return Math.sqrt(
                    ...
                    Math.pow(a.Kidney_Function_Score - b.Kidney_Function_Score, 2)
                );
        ...
        const neighbors = distances.sort((a, b) => a.distance - b.distance).slice(1, neighborCount + 1);
